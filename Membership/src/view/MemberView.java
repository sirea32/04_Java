...
public class MemberView {
	...
	public void mainMenu() {
		...
		do {
			...
			try {
				...
				System.out.println("\n### 숫자만 입력 해주세요 ###\n");
				input = -1; // 첫 반복에서 종료되지 않게 값 변경(그냥 아무런 숫자를 넣은거임 -1 도 아무런숫자)
		...
		}while(input != 0);    // input 0이 아닐때까지 반복
	...
	private void selectName() throws IOException {
		...
		String searchName = br.readLine();
							// readLine을 하면 예외 발생할 가능성이 생김 예외를 던져라! throws
		
		// 이름 검색 서비스 호출후 결과 반환 받기
	...
	private void updateAmount() throws IOException{
		...
		if(searchList.size() > 1) {
			...
			// 일반 for문 사용 이유 -> i 값을 활용하고 싶어서
			for(int i=0; i<searchList.size(); i++) {
				System.out.printf("(%d) %s (%s)\n",
						i+1, searchList.get(i).getName(),
